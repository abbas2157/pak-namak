...
@section('title', 'Spice Sales Report')
...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2 align-items-center">
            <div class="col-sm-6">
                <h1 class="m-0">Spice Sales Report <small class="text-muted">مصالحہ فروخت رپورٹ</small></h1>
                <ol class="breadcrumb mt-1">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                    <li class="breadcrumb-item active">Spice Sales Report</li>
                </ol>
            </div>
            ...
            <div class="card-body py-3">
                <div class="row align-items-end">
                    ...
                    <div class="col-md-3 mb-2 mb-md-0">
                        ...
                        @if($from || $to)
                            <a href="{{ route('admin.spice-sales.report') }}" class="btn btn-outline-secondary btn-modal-cancel ml-2">
                    ...
                    <div class="col-md-3 text-muted">
                        ...
                        @else
                            Showing all spice sales &nbsp;·&nbsp; <strong>{{ $sales->count() }}</strong> total
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-3 col-sm-6 mb-3">
                <div class="card border-0 shadow-sm h-100 card-pn bg-light">
                    <div class="card-body py-3 px-4">
                        <div class="pn-form-col-lbl text-uppercase">By Spice Type / قسم کے مطابق</div>
                        <div class="mt-1">
                            @forelse($byType as $type)
                                <div class="d-flex justify-content-between text-muted">
                                    <span class="font-weight-bold">{{ $type->type_name }}</span>
                                    <span>PKR {{ number_format($type->total, 0) }}</span>
                                </div>
                            @empty
                                <div class="text-muted">No spice sales yet</div>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            <div class="card-header border-0 py-3 ch-blue">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h6 class="mb-0 font-weight-bold text-c-blue2">Revenue by Spice Type / قسم کے مطابق آمدن</h6>
                    </div>
                    <span class="badge badge-primary bdg-md">{{ $byType->count() }} types</span>
                </div>
            </div>
            <div class="card-body p-0">
                @php $maxTypeTotal = $byType->max('total') ?: 1; @endphp
                <div class="table-responsive">
                <table class="table table-sm mb-0 pn-table pn-table-font">
                    <thead>
                        <tr>
                            <th class="pl-3">#</th>
                            <th>Spice Type / قسم</th>
                            <th class="text-center">Packets / پیکٹ</th>
                            <th class="text-right">Total KG / کل کلو</th>
                            <th class="text-right pr-3">Revenue (PKR) / آمدن</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($byType as $i => $type)
                            <tr class="pn-table-row">
                                <td class="pl-3 text-muted pn-stat-sub">{{ $i + 1 }}</td>
                                <td>
                                    <div class="font-weight-bold">{{ $type->type_name }}</div>
                                    <div class="progress breakdown-prog-3 mt-1" style="width:120px;">
                                        <div class="progress-bar pbar pbar-blue"
                                             style="--w:{{ ($type->total / $maxTypeTotal) * 100 }}%"></div>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <span class="badge badge-light border">{{ number_format($type->packets, 0) }}</span>
                                </td>
                                <td class="text-right">{{ number_format($type->total_kg, 2) }}</td>
                                <td class="text-right pr-3 font-weight-bold text-c-blue2">
        ...
        <div class="card border-0 shadow-sm mb-4 card-pn">
            <div class="card-header border-0 py-3 ch-blue">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <div class="pn-form-col-lbl text-uppercase">Summary</div>
                        <h6 class="mb-0 font-weight-bold text-c-blue2">Spice Sales by Shop / دکان کے مطابق</h6>
                    </div>
                    <span class="badge badge-primary bdg-md">{{ $salesByShop->count() }} shops</span>
                </div>
            </div>
            <div class="card-body p-0">
                ...
                <table class="table table-sm mb-0 pn-table pn-table-font">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="6" class="text-center py-4 text-muted">No spice sales found for the selected period.</td></tr>
                        @endforelse
                    </tbody>
            ...
            <div class="card-header border-0 py-3 ch-teal">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h6 class="mb-0 font-weight-bold text-c-teal">All Spice Sales / تمام فروخت</h6>
                    </div>
                    <span class="badge badge-success bdg-md">{{ $sales->count() }} records</span>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    ...
                    <tbody>
                        @forelse($sales as $i => $sale)
                            <tr class="pn-table-row">
                                <td class="pl-3 text-muted pn-stat-sub">{{ $i + 1 }}</td>
                                <td>
                                    <div class="font-weight-bold">{{ $sale->shop->name ?? '—' }}</div>
                                ...
                                <td class="text-center pr-3 no-print">
                                    <a href="{{ route('admin.spice-sales.receipt', ['id' => $sale->id]) }}"
                        ...
                        @empty
                            <tr><td colspan="7" class="text-center py-4 text-muted">No spice sales found for the selected period.</td></tr>
                        @endforelse
                    </tbody>
                </table>
                </div>
...
<script>
    function applyFilter() {
        ...
        window.location.href = '{{ route('admin.spice-sales.report') }}' + (query ? '?' + query : '');
    }
</script>
@endsection
